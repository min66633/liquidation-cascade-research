...
연구 기간 2026-07 ~ 2026-08-22.

## 결론

급락(청산 캐스케이드) 직후 매수의 반등 우위는 통계적으로 실재한다 — 워크포워드
표본외 +15.5bp/거래, 진입 시각만 무작위인 대조 거래 289,715개 대비 +22.7bp(t=11.2).
그러나 그 우위가 발생하는 순간은 정확히 호가창 유동성이 사라지는 순간이라, 체결
비용을 실측하면 무레버리지 수익 상한이 연 3.4%로 무위험 수익률 수준이다.
실거래 자본 투입 근거 없음으로 판정하고 연구를 종료했다.
...
"그 바닥을 사면 되지 않나"라는 오래된 직관이 있다. 그 직관을 끝까지 계산한
기록이다.
...
검증 과정에서 백테스트 결함이 하나 나왔다. 중간 버전에서 샤프 2.38이 나왔는데,
